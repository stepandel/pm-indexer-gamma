...
import type { markets, market_events, tags, Prisma } from "@prisma/client";
import type { MarketEvent, Market, Tags } from "../types/market";

const upsertMarketEvent = async (
  eventData: any
): Promise<market_events | null> => {
  if (!database?.prisma) {
  ...
  try {
    ...
    logger.error("Failed to upsert market event", {
...
const saveEventWithMarkets = async (
  event: MarketEvent
): Promise<{
  event: market_events | null;
  markets: markets[];
  tags: tags[];
}> => {
  ...
  try {
    // Save event first
    const savedEvent = await upsertMarketEvent({
      id: event.id || "",
    ...
    logger.error("Failed to save event with markets and tags", {
      eventId: event.id,
      error,
    });
    throw error;
